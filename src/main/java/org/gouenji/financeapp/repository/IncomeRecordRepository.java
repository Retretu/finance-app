...
public interface IncomeRecordRepository extends JpaRepository<IncomeRecord, Integer> {
    ...
    @Query("SELECT i FROM IncomeRecord i WHERE YEAR(i.date) = :year AND MONTH(i.date) = :month")
    List<IncomeRecord> findAllByYearAndMonth(@Param("year") int year, @Param("month") int month);
}
